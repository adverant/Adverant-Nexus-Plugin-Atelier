/**
 * Usage Tracking Middleware for Atelier
 *
...
function detectOperation(req: Request): string {
  ...
  // Atelier-specific operations
  if (path.includes('/video')) return 'video_generation';
  if (path.includes('/image')) return 'image_generation';
  if (path.includes('/audio')) return 'audio_generation';
  if (path.includes('/render')) return 'render';
  if (path.includes('/job')) return 'job_operation';
  if (path.includes('/asset')) return 'asset_operation';
  if (path.includes('/template')) return 'template_operation';
  if (path.includes('/effect')) return 'effect_operation';
  if (path.includes('/transition')) return 'transition_operation';
  if (path.includes('/composite')) return 'composite_operation';
